...
%   integrand_int_math = math(['InputForm[D[',integrand_math,',',var,']]']);
  ...
  % If convert to sym, messed up with one of the cone moments. Get MuPad output
  % such as arccos, etc. Don't know why this does it for one but not the other.
%   syms_str_out(ctr1,ctr2) = sym(integrand_int_str); - old way
%   syms_str_out{ctr1,ctr2} = integrand_int_str;
