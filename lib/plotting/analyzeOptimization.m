    ...
    for ctrConstant = 1 : 1 : length(constNames)
		...
		for ctrPlot = 1 : 1 : length(in.figure)
          ...
          for ctrPlotExpression = 1 : 1 : numPlotExpressions
            xFunc{ctrRow,ctrCol,ctrPlotExpression} = matlabFunction(str2sym(in.figure(ctrPlot).plot(indPlot).x{ctrPlotExpression}{1}),'vars',inputSet);
            yFunc{ctrRow,ctrCol,ctrPlotExpression} = matlabFunction(str2sym(in.figure(ctrPlot).plot(indPlot).y{ctrPlotExpression}{1}),'vars',inputSet);
          end
