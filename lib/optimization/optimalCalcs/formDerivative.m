function [derivative] = formDerivative(expression,variables)
	...
	for ctrVariable = 1 : 1 : length(variables)
		...
		if ~contains(char(derivative(ctrVariable)),'D(') && ...
        ~contains(char(derivative(ctrVariable)),'[1]') && ...
        ~contains(char(derivative(ctrVariable)),'diff') % Analytic derivative found, finish. Will usually find [1] in Mathematica numerical D[] result.
