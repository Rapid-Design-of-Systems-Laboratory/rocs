...
%         options_bvp = in.bvpOptions;
        ...
        if in.skipUnconvergedSolutions % skipping activated
            try
                ...
                if (in.rootSolving == 0) % using bvp4c
                    ...
                    if length(in.vars.arcTypeSequence) == 1
                        options_bvp = bvpset('AbsTol',const.tol,'RelTol',const.tol,'Nmax',const.NMax,'Stats','on');
            ...
            if (in.rootSolving == 0) % using bvp4c
                ...
                if length(in.vars.arcTypeSequence) == 1
                    options_bvp = bvpset('AbsTol',const.tol,'RelTol',const.tol,'Nmax',const.NMax,'Stats','on');
%                     solInit.parameters(1) = 33*3600*25/(const.T*6378^1.5/sqrt(398600.4418));
                    %disp('yay')
%                     cd ./data
%                     load('fsolve_ode_res.mat')
%                     cd ..
%                     solInit.x = Tp./Tp(end);
%                     solInit.y = Xp';
%                     solInit.parameters(1) = Tp(end);
%                     solInit.parameters(2) = Xp(1,8);
% 					solInit.parameters(3) = Xp(1,9);
% 					solInit.parameters(4) = Xp(1,10);
% 					solInit.parameters(5) = Xp(1,11);
% 					solInit.parameters(6) = Xp(1,12);
% 					solInit.parameters(7) = Xp(1,13);
% 					solInit.parameters(8) = Xp(1,14);
%                     solInit.parameters(9) = Xp(end,8);
% 					solInit.parameters(10) = Xp(end,9);
% 					solInit.parameters(11) = Xp(end,10);
% 					solInit.parameters(12) = Xp(end,11);
% 					solInit.parameters(13) = Xp(end,12);
                    sol = bvp4c(derivFunc,bcFunc,solInit,options_bvp,const,constraintVal,in.vars.arcTypeSequence,in.vars.interiorPointConstraintSequence,oc.num.lagrangeMultipliers.interiorPoint,x0,xf);
                    if in.useDeval
                        tau = linspace(0,1,const.NMesh);
                        sol.y = deval(sol,tau);
        ...
        % Output to screen
        if ~sol.errorFlag
            fprintf('Continuation solution %d/%d converged of continuation set #%i in %g seconds.\n\n',ctrContinuationChange,numRuns,setCtr,runTime);
