...
% output : oc [structure]
% Developed by : Dr. M.J. Grant, Dr. K. Mall, and Dr. T. Antony
...
oc.state.rate = str2sym(in.oc.stateRate(:,1)); % Process equations
...
oc.cost.path = str2sym(in.oc.cost.path{1,1});
...
oc.constraint.initial = str2sym(in.oc.constraint.initial(:,1));
...
oc.cost.augmented.terminal = str2sym(in.oc.cost.terminal{1});
...
if isfield(in.oc.constraint,'terminal')
    ...
    % Terminal constraint
    oc.constraint.terminal = str2sym(in.oc.constraint.terminal(:,1));
...
for ctrTerminalConstraint = 1 : 1 : oc.num.lagrangeMultiplier.terminal
    oc.cost.augmented.terminal = oc.cost.augmented.terminal + ...
        ['lmTerminal',int2str(ctrTerminalConstraint)]*oc.constraint.terminal(ctrTerminalConstraint);
end
...
oc.hamiltonian.unconstrained.expression
...
if in.oc.writeEquations
    ...
    for ctrControl = 1 : 1 : oc.num.controls
        %         if ((contains(char(oc.hamiltonian.unconstrained.controlPartial(ctrControl)), ['sin(',char(oc.control.var(ctrControl)),')']) || ...
        %              contains(char(oc.hamiltonian.unconstrained.controlPartial(ctrControl)), ['cos(',char(oc.control.var(ctrControl)),')']))...
        %              && contains(char(oc.hamiltonian.unconstrained.controlPartial(ctrControl)), char(oc.control.var(ctrControl))))
        %              error(['Transcendental equation encountered. Use time-derivative of ',char(oc.control.var(ctrControl)),' as the new control and make ',char(oc.control.var(ctrControl)),' as a state. For more details, refer to the documentation of the UTM.'])
        %         end
        %
        
        %%  Bounded control law
        if (contains(char(oc.hamiltonian.unconstrained.controlPartial(ctrControl)), ['cos(',char(oc.control.var(ctrControl)),')']))               
            % Optimal control problems with non-linear control constraints
            oc.hamiltonian.unconstrained.costerm = diff(str2sym(strrep(char(oc.hamiltonian.unconstrained.controlPartial(ctrControl)),['cos(',char(oc.control.var(ctrControl)),')'],['cos',char(oc.control.var(ctrControl))])),str2sym(['cos',char(oc.control.var(ctrControl))]));
%             oc.hamiltonian.unconstrained.costerm = diff(str2sym(strrep(char(oc.hamiltonian.unconstrained.controlPartial(ctrControl)),cos_cont,cos_cont)),cos_cont);
            % For bang-bang and singular controls
            if (contains(char(oc.hamiltonian.unconstrained.controlPartial(ctrControl)), 'epsilon')) 
                fprintf([char(oc.control.var(ctrControl)),' corresponds to a bang-bang and/or singular control.\n'])
                % Write the two control options for the bang-bang and
                % singular control law
                oc.control.unconstrained.expression{ctrControl}{1,1} = ['atan((',char(oc.hamiltonian.unconstrained.costerm),')/epsilon)'];
                oc.control.unconstrained.expression{ctrControl}{2,1} = ['atan((',char(oc.hamiltonian.unconstrained.costerm),')/epsilon) + pi'];
%             if (contains(char(oc.hamiltonian.unconstrained.controlPartial(ctrControl)), ['cos(',char(oc.control.var(ctrControl)),')*sin(',char(oc.control.var(ctrControl)),')']))             
            else % for constraints upon non-linear controls
                fprintf([char(oc.control.var(ctrControl)),' corresponds to a non-linear constrained control.\n'])
                % Write the three control options for the non-linear
                % constrained control law
                oc.control.unconstrained.expression{ctrControl}{1,1} = '-pi/2';
                oc.control.unconstrained.expression{ctrControl}{2,1} = 'pi/2';
                asinterms = simplify(solve(oc.hamiltonian.unconstrained.costerm == 0,oc.control.var(ctrControl)));
                oc.control.unconstrained.expression{ctrControl}{3,1} = char(asinterms(1));
            end
            
        else
            fprintf([char(oc.control.var(ctrControl)),' is an unconstrained control.\n'])
            fprintf('Using solve function of MATLAB to find the control law.\n')
            % Use MATLAB's solv to find the control law
                   oc.control.unconstrained.expression{ctrControl} = ...
                   		simplify(solve(oc.hamiltonian.unconstrained.controlPartial(ctrControl)==0,oc.control.var(ctrControl)));
           ...
           % The following line of code uses Matematica to get high quality solutions
...
%                 sym('0'),oc.control.var(ctrControl),in.oc.assumptions.control,in,oc);
    ...
    for ctrExpression = 1 : 1 : oc.num.controls
        for ctrControl = 1 : 1 : oc.num.controls
            ...
            if contains(char(oc.control.unconstrained.expression{ctrExpression}(1)),char(oc.control.var(ctrControl)))
...
for ctrPathConstraint = 1 : 1 : oc.num.constraints.path
    ...
    while true
        ...
        for ctrControl = 1 : 1 : oc.num.controls
            ...
            if contains(char(constraintExpression),char(oc.control.var(ctrControl)))
                controlFound = true;
    ...
    if in.oc.writeEquations
        ...
        for ctrExpression = 1 : 1 : oc.num.controls
            for ctrControl = 1 : 1 : oc.num.controls
                ...
                if contains(char(oc.control.constraint.path{ctrPathConstraint}.expression{ctrExpression}(1)), ...
