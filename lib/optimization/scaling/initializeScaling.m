...
for ctrScale = 1 : 1 : length(in.scale(:,1))
	...
	elseif ~isempty(strfind(in.scale{ctrScale,2},'.')) % value from structure in input file
		...
		in.vars.scaleValFn{ctrScale,2} = matlabFunction(str2sym(in.scale{ctrScale,2}),'vars',sym('x',[out.oc.num.states 1]));
	end
end
...
for ctrConst = 1 : 1 : length(names)
	units = str2sym(in.const.(char(names{ctrConst})){2});
...
for ctrState = 1 : 1 : out.oc.num.states
	units = str2sym(in.oc.state{ctrState,2});
...
for ctrCostate = 1 : 1 : out.oc.num.states
	units = str2sym(['(',in.oc.cost.path{2},')/(',in.oc.state{ctrCostate,2},')']);
...
for ctrConstraint = 1 : 1 : out.oc.num.constraints.initial
	...
	in.vars.scaleFn.lagrangeMultipliers.initial{ctrConstraint} = matlabFunction(str2sym(units),'vars',unitSym);
end

for ctrConstraint = 1 : 1 : out.oc.num.constraints.terminal
	units = ['(',in.oc.cost.path{2},')/(',in.oc.constraint.terminal{ctrConstraint,2},')'];
	in.vars.scaleFn.lagrangeMultipliers.terminal{ctrConstraint} = matlabFunction(str2sym(units),'vars',unitSym);
end
