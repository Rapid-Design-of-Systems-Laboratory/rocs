...
switch in.w_type
  ...
  case 'arch'
    ...
    if od.flag(od.iter-1) == 1
      od.w(od.iter) = od.w(od.iter-1) * 0.95;
    end
...
if od.iter == 1
  ...
  if isempty(C)
    fprintf('  Warning! No solutions in archive!');
  end
  ...
  % GFM: modify vel limit as inertia changes
  temp_w = 1 / max([1/3 od.w(od.iter)]);
  temp_vel = in.vel_min(:,ones(size(od.vel,2),1)) * temp_w;
  ...
  % GFM: modify vel limit as inertia changes
  temp_vel = in.vel_max(:,ones(size(od.vel,2),1)) * temp_w;
...
if od.iter == 1
  ...
  if od.num_constr > 0
    ...
    while ~isempty(C) & in.max_iter > 1
      % make list of feasible solutions and pick which ones to use
      G = setdiff(1:in.num_ind,C);
      dC = ceil(rand(size(C)) * length(G));

      if 1 %~isempty(G)
        % GFM: modified to randomly use feasible positions (different velocities)

         % Output to screen
        fprintf(['%g/%g (%6.3g%%) infeasible solutions. Reinitializing infeasible ', ...
          'solutions.\n'],length(C),in.num_ind,length(C)/in.num_ind*100);

         % replace position/fitness/constraint data with feasible solutions
         od.pop(:,C,od.iter)      = od.pop(:,G(dC),od.iter);
         od.fit_save(:,C,od.iter) = od.fit_save(:,G(dC),od.iter) ...
                                  + mean(od.fit_save(:,G(dC),od.iter),2) ... 
                                  * rand(size(dC)) * -1e-6; 
         od.constr(:,C,od.iter)   = od.constr(:,G(dC),od.iter);
         ...
         % error check
         if ~isempty(C)
           error('adv_pop: bad infeasible update');
         end;

      else
        % original Mike
        % Iterate again until have a feasible solution
  ...
  % Bounce at current speed, 50% of time
  I_bounce = find(value == 1);
  od.vel(I_bounce+index_bias) = -od.vel(I_bounce+index_bias);
  
  % Bounce away at 10% max, 50% of time (GFM)
  I_bounce = find(value == 0);
  temp_vel = in.vel_max(:,ones(size(od.vel,2),1));
  od.vel(I_bounce+index_bias) = -sign(od.vel(I_bounce+index_bias)) ...
                              .* 0.1 .* temp_vel(I_bounce);
  
  % Set velocity to zero, 25% of time
  % I_stop = find(value == 0);
