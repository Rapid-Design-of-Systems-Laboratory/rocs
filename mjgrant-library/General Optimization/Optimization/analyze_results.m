...
if in.num_obj == 1
  ...
  if od.dim ~= 1
    ...
    [X,Y] = meshgrid(xg,yg);
    fitness_plot = -X.^2 - Y.^2 + 2*X + Y + 100*sin(X) + 200*sin(Y);
...
%     aviobj = avifile('optimization_movie.avi','Compression','Indeo5','FPS',8);
    aviobj = avifile('optimization_movie','Compression','Cinepak','FPS',8);
  end
...
%       axis([1 od.iter in.vel_min(dim_counter) in.vel_max(dim_counter)]);
  ...
  % Inertia plot
  subplot('position',pos2); hold on; grid on; set(gca,'box','on');
  title('Inertia vs. Iteration');
  xlabel('Iteration'); ylabel('Inertia');
  plot([1:1:od.iter],od.w(1:od.iter));
  axis_info = get(gca);
  ...
  for counter = 1 : 1 : od.iter
    ...
    for in_counter = 1 : 1 : in.num_ind
      h1(in_counter) = plot(od.pos_arch(1,in_counter,counter),od.pos_arch(2,in_counter,counter),'b.');
      hold on; grid on;
      h2(in_counter) = plot(od.pop(1,in_counter,counter),od.pop(2,in_counter,counter),'rx');
      axis([-5 5 -5 5]);
      plot([-5 0],[-5 0],'k');
%       % Plot front
...
%       h2(in_counter) = plot([1:1:od.dim],od.pop(:,in_counter,counter),'r');
    ...
    if in.num_obj == 2
      ...
      ytemp=xtemp;
      j1temp=xtemp+ytemp;
      j2temp=xtemp.^2+ytemp.^2;
      plot(j1temp,j2temp,'k');
      axis([-10 0 0 50]);
    elseif in.num_obj == 3
      zlabel('J_3');
      axis([trade_min(1) trade_max(1) trade_min(2) trade_max(2) ...
