...
num_neighbors = 4;  % Number of neighbors that will communicate with each particle
maxIterations = 500;  % Maximum number of iterations to perform
...
delFitness = 10;  % Fitness-determined termination criteria
delIter = 5;  % Determines number of iterations to change from global to local search

global_iter = 10;
best_save = 0;
global_search = true; % Start with global searching in mind
best_criteria = -10000;
global_tol = 1000;
...
best_fitness = -10000*ones(1,maxIterations); % Vector used to store the best fitness of each generation.
...
while iteration <= maxIterations
  ...
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   ...
   if (iteration ~= 1) && (abs(best_fitness(iteration)-best_fitness(iteration-1)) <= delFitness)
     terminate_counter = terminate_counter + 1;
     if terminate_counter == delIter
       fprintf('\n\nBREAK\n\n');
       break;
     end
   else
   ...
   % Establish local vs. global search
   if global_search && (abs((max(bestSoFar) - best_criteria)) > global_tol)
     best_criteria = max(bestSoFar);
     global_search_counter = 0;
   elseif global_search
     global_search_counter = global_search_counter + 1
     if global_search_counter == global_iter
       global_search == false
       fprintf('\n\nGLOBAL SEARCH FALSE HERE\n\n');
     end
   end
   global_search
   if (global_search == false)% | (global_search_counter == global_iter)
    fprintf('\n\nGLOBAL SEARCH FALSE\n');
    return;
  end
pause
  if global_search
    % Global searching inertia
    w = 10*(1.2 - iteration/maxIterations);
  else
    % Local searching inertia
    w = (1.2 - iteration/maxIterations);
  end
  ...
  r1 = rand;
  r2 = rand;
%   w = 10*(1.2 - iteration/maxIterations); % Inertia
...
if nargout == 0
  ...
  fprintf('\nBest Fitness Attained in Iteration %d out of %d',min(I),iteration);
