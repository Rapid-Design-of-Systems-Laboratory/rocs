function [y,varargout] = min(x,varargin)
% CADA overloaded version of function MIN
%
% Copyright 2011-2014 Matthew J. Weinstein and Anil V. Rao
% Distributed under the GNU General Public License version 3.0
global ADIGATOR
if nargout == 2
  error('min only coded for single output')
end
...
elseif nargin == 2
  y = cadabinaryarraymath(x,varargin{1},0,0,'min');
  return
elseif nargin == 3
  if isa(x,'cada')
    xMrow = x.func.size(1); xNcol = x.func.size(2);
  ...
  if isa(x2,'cada') || ~isempty(x2)
    error('three input syntax for min: y = min(x,[],dim) -- second input is wrong');
  end
  Dim = varargin{2};
  if isa(Dim,'cada')
    if ~isempty(Dim.func.value)
      ...
      error('??? cannot find min across a purely symbolic dimension')
    end
...
elseif isinf(xMrow) || isinf(xNcol)
  error('Cannot find min over vectorized dimension')
else
  xvec = 0;
end
...
if ~isempty(x.func.value)
  % Y is numeric
  y.func.value = min(x.func.value,Dim);
end
...
if cadaCheckForDerivs(x)
  ...
  if DPFLAG
    funprintflag = 1;
    fprintf(fid,[indent,TF1,' = min(',x.func.name,',[],%1.0f);\n'],Dim);
  end
  TF2 = ['cada',NDstr,'tf2'];
  TF3 = ['cada',NDstr,'tf3'];
  TD3 = ['cada',NDstr,'td3'];
  if Dim == 1
    % finding min over first dimension
    A.func.size = [1 xMrow];
    if xNcol == 1 && ~xvec
      % y ~ A*x
    ...
    % finding min over second dimension
    A.func.size = [xNcol 1];
    if xMrow == 1 && ~xvec
      % y ~ x*A
...
if PFLAG && ~funprintflag
  if nargin == 1
    fprintf(fid,[indent,funcstr,' = min(',x.func.name,');\n']);
  else
    fprintf(fid,[indent,funcstr,' = min(',x.func.name,',[],%1.0d);\n'],Dim);
  end
end
