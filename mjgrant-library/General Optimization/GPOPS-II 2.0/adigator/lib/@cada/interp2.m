...
%
% Update V0.4 - this will now call: 
% pp2 = adigatorGenInterp2pp(x,y,z,method);
...
if isempty(x.func.value) || isempty(y.func.value) || isempty(z.func.value)
  ...
  if ADIGATOR.FORINFO.FLAG && PFLAG
    errstr = ['Cannot use interp2 within a loop if the values of ',...
      'X, Y, and Z change on each iteration - please use adigatorGenInterp2pp ',...
      'to build interp2 polynomials PRIOR to differentiation and input them as ',...
      'known numeric inputs adigatorEvalInterp2pp will then allow for you to ',...
      'loop on the different generated polynomials - see manual for more information'];
    error(errstr)
  else
    error('Inputs X, Y, and Z to interp2 must be known numeric values')
  end
  % WAS ATTEMPTING TO ALLOW FOR THIS BUT IT WAS DIFFICULT AS THE VALUES OF
  % X, Y, and Z MAY CHANGE DIMENSIONS
  
%   if cadaCheckForDerivs(x) || cadaCheckForDerivs(y) || cadaCheckForDerivs(z)
%     error('Inputs X, Y, and Z to interp2 must not contain derivatives')
%   end
else
  %ppknown = 1;
...
%     pp.yorder = 4;
%   end
%   if isempty(x.func.name)
%     x.func.name = cadamatprint(x.func.value);
%   end
%   if isempty(y.func.name)
%     y.func.name = cadamatprint(y.func.value);
%   end
%   if isempty(z.func.name)
%     z.func.name = cadamatprint(z.func.value);
%   end
%   ppStr = ['cada',NDstr,'pp2'];
%   fprintf(fid,[indent,ppStr,' = adigatorGenInterp2pp(',...
%     x.func.name,',',y.func.name,',',z.func.name,',''',method,''');\n']);
% end
...
for Vcount = 1:NUMvod
  ...
  if DPFLAG && ~ppxflag && ~isempty(xi.deriv(Vcount).nzlocs)
    ...
    %if ppknown
    ppxStr  = getppderiv(pp,'x');
%     else
%       ppxStr  = getppderiv(pp,'x',ppStr,fid,indent,NDstr);
%     end
  ...
  if DPFLAG && ~ppyflag && ~isempty(yi.deriv(Vcount).nzlocs)
    %if ppknown
    ppyStr  = getppderiv(pp,'y');
    %else
    %  ppyStr  = getppderiv(pp,'y',ppStr,fid,indent,NDstr);
    %end
