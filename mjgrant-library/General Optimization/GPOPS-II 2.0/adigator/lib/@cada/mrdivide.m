...
elseif xNcol == yNcol
  FMrow = xMrow;
  FNcol = yMrow;
  if yMrow ~= yNcol && (cadaCheckForDerivs(x) || cadaCheckForDerivs(y))
    zid = ADIGATOR.VARINFO.COUNT;
    if PFLAG
      zvarinfo = ADIGATOR.VARINFO.NAMELOCS(zid,:);
      if ADIGATOR.VARINFO.NAMELOCS(zid,1) > 0
        if ADIGATOR.VARINFO.NAMELOCS(zid-1,1) > 0
          varcount = 1;
        else
          varcount = ADIGATOR.VARINFO.NAMELOCS(zid-1,2) + 1;
        end
      else
        varcount = ADIGATOR.VARINFO.NAMELOCS(zid,2)+1;
      end
      ADIGATOR.VARINFO.NAMELOCS(zid,:) = [0 varcount 0];
      v1 = x.';
      ADIGATOR.VARINFO.COUNT = zid;
      ADIGATOR.VARINFO.NAMELOCS(zid,:) = [0 varcount 0];
      v2 = y.';
      ADIGATOR.VARINFO.COUNT = zid;
      ADIGATOR.VARINFO.NAMELOCS(zid,:) = [0 varcount 0];
      v3 = v2\v1;
      ADIGATOR.VARINFO.COUNT = zid;
      ADIGATOR.VARINFO.NAMELOCS(zid,:) = zvarinfo;
      z  = v3.';
    else
      v1 = x.';
      ADIGATOR.VARINFO.COUNT = zid;
...
for Vcount = 1:NUMvod;
  ...
  if ~isempty(y.deriv(Vcount).nzlocs)
    ...
    Bnzlocs = cadamtimesderiv(A,y,Atemp,ytemp,Vcount,TD3,DPFLAG,'mtimes');
    % dim(A) = dim(z), so, dim(A*Y) = [zMrow yNcol] = dim(X)
    B.func = x.func; B.func.name = []; Btemp = xtemp; %Btemp never really gets used
    B.deriv = A.deriv;
