...
if ~ADIGATOR.OPTIONS.UNROLL
  ...
  ADIGATORFORDATA(ForCount).COUNT.STRUCTREF  = 0;
  ADIGATORFORDATA(ForCount).COUNT.STRUCTASGN = 0;
...
elseif ADIGATOR.RUNFLAG == 1
  ADIGATOR.VARINFO.COUNT = ADIGATORFORDATA(ForCount).START;
  ADIGATOR.PREOPCOUNT    = ADIGATOR.VARINFO.COUNT;
  if ~isempty(ADIGATOR.VARINFO.OVERMAP.CONT) && ~ADIGATORFORDATA(ForCount).WHILEFLAG
