...
function dydx = eoms_JacobsonLele_trig(x,y,p)
global x1Max scale
% Derivative function 
dydx = zeros(10,1);
...
dydx(1,1) = y(2)/(x1Max*(cos(y(1))));
%dydx(1,1) = y(2)*pi*(1+y(1)^2)/(2*scale*x1Max);
dydx(2,1) = y(3);
dydx(3,1) = y(4);
dydx(4,1) = u;
dydx(5,1) = 1;
% dydx(6,1) = 0;
% dydx(7,1) = -y(6);
dydx(6,1) = -y(6)*sin(y(1))*y(2)/(x1Max*(cos(y(1))^2));
dydx(7,1) = -y(6)/(x1Max*(cos(y(1))));
dydx(8,1) = -y(7);
dydx(9,1) = -y(8);
dydx(10,1) = 0;
