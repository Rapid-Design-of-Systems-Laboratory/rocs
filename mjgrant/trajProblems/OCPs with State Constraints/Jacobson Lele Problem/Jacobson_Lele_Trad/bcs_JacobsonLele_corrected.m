...
function res = bcs_JacobsonLele_corrected(YL,YR, p)
...
ham1_minus = YR(5,1)*YR(2,1) + YR(6,1)*YR(3,1) + YR(7,1)*YR(4,1) + YR(8,1)*u_init + 0.5*u_init^2;
ham2_plus = YL(5,2)*YL(2,2) + YL(6,2)*YL(3,2) + YL(7,2)*YL(4,2) + YL(8,2)*0 + 0.5*0^2;
ham3_minus = YR(5,2)*YR(2,2) + YR(6,2)*YR(3,2) + YR(7,2)*YR(4,2) + YR(8,2)*0 + 0.5*0^2;
ham4_plus = YL(5,end)*YL(2,end) + YL(6,end)*YL(3,end) + YL(7,end)*YL(4,end) + YL(8,end)*u_end1 + 0.5*u_end1^2;
ham_end = YR(5,end)*YR(2,end) + YR(6,end)*YR(3,end) + YR(7,end)*YR(4,end) + YR(8,end)*u_end2 + 0.5*u_end2^2;
...
res = [ YL(1,1) - 0 % x1(0) = 0
    ...
    % YL(5,1) - 0 % t(0) = 0
    YL(5,1) - p(4) % lamx1(0) = nu_x1_0
    YL(6,1) - p(5) % lamx2(0) = nu_x2_0
    YL(7,1) - p(6) % lamx3(0) = nu_x3_0
    YL(8,1) - p(7) % lamx4(0) = nu_x4_0
    ...
    % YR(5,end) - 10 % t(10) = 10
    YR(5,end) - p(8) % lamx1(10) = nu_x1_f
    YR(6,end) - p(9) % lamx2(10) = nu_x2_f
    YR(7,end) - p(10) % lamx3(10) = nu_x3_f
    YR(8,end) - p(11) % lamx4(10) = nu_x4_f
    ...
    YR(5,1) - YL(5,2) - p(12)% Discontinuity of lamh at entry
    YR(6,1) - YL(6,2) - p(13)% Continuity of lamv at entry
    YR(7,1) - YL(7,2) - p(14) % Continuity of lamt at entry Note: No jump
    YR(8,1) - YL(8,2) - p(15) % Continuity of lamt at entry Note: No jump
    % YR(10,1) - YL(10,2) - 0 % Continuity of lamt at entry Note: No jump 
    YR(5,2) - YL(5,3) - 0% Continuity of lamh at exit
    YR(6,2) - YL(6,3) - 0% Continuity of lamv at exit 
    YR(7,2) - YL(7,3) - 0% Continuity of lamt at exit 
