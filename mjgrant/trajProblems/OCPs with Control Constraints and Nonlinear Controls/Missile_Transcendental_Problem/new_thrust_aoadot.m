...
in.oc.stateRate = {'v*sin(gam)'; ...
				'v*cos(gam)/(re+h)'; ...
                ['(',T,'*cos(',alfa,') -',D,')/mass - mu*sin(gam)/(re+h)^2+ epsilon*cos(alfadottrig)']; ...
                ['(',T,'*sin(',alfa,') +',L,')/(mass*v) + (v/(re+h) - mu/(v*(re+h)^2))*cos(gam)'];...
                ['-',T,'/(g0*',Isp,')'];...
                'alfarate*sin(alfadottrig)'};

%%%%%%%%%%%%%
%% Control %%
%%%%%%%%%%%%%

in.oc.control = {'alfadottrig','rad'};
...
in.const.alfamax = {50,'rad'};
in.const.Tmax  = {2000000,'kg*m/s^2'}; 
in.const.Isp  = {400,'s'}; % 1600
in.const.g0  = {9.80665,'m/s^2'};
in.const.alfarate  = {5*pi/180,'rad/s'};
in.const.epsilon  = {1,'m/(s*rad)'};
...
in.oc.guess.initial.alfa = 0*pi/180;
in.oc.guess.costate = -1;%-0.1;
%%%%%%%%%%%%%%%%%%
%% Continuation %%
%%%%%%%%%%%%%%%%%%
...
% in.CONT{ind}.constraint.terminal.thetta = 300/6378;
...
% in.cont.method(ind) = 1;
% 
% in.CONT{ind}.numCases = 500; % Number of steps in the continuation set
% in.CONT{ind}.const.epsilon = linspace(0,-0.999,in.CONT{ind}.numCases); 
