function [in] = Aircraft_Noise_3DOF_pop_new_x() 
...
in.bvpOptions = bvpset('AbsTol',1e-9,'RelTol',1e-9,'Nmax',10000000000,'Stats','on');
...
bank = '(bankmax*sin(banktrignew))';
alfa = '(alfamax*sin(alfatrignew))';
D = '(C1*v^2+C2/(v^2))';
L = '(mass*g)';
T = '(1560*sin(Ttrigpopnew_x)+1860)';
...
in.oc.control = {'banktrignew','rad';...
                 'alfatrignew','rad';...
                 'Ttrigpopnew_x','kg*m/s^2'};
...
% Path cost
in.oc.cost.path = {[pop,'*',T,'^5.2*cos(gam)/(v*(z+50)^2.5)'],'kg*m^1.7/s^9.4'};

% Terminal cost
in.oc.cost.terminal = {'0','kg*m^1.7/s^9.4'};

% Initial cost
in.oc.cost.initial = {'0','kg*m^1.7/s^9.4'};
...
in.oc.guess.timeIntegrate = 30; % 50
...
in.CONT{ind}.constraint.terminal.x = 5400; % 5400
