...
bank = '(60*pi/180*sin(banktrignew))';
...
% ind = ind+1;
% in.figure(ind).plot(1).x{1} = {'t','s','Time'};
% in.figure(ind).plot(1).y{1} = {'60*sin(banktrignew)','deg','Bank Angle'};
% in.figure(ind).plot(1).legend.location = 'NorthWest';
% in.figure(ind).plot(2).x{1} = {'t','s','Time'};
...
ind = ind+1;
in.figure(ind).plot(1).x{1} = {'t','s','Time'};
in.figure(ind).plot(1).y{1} = {'lamZ*v*sin(gam) - lamV*((C1*v^2 + C2/v^2 - cos(alfamax*sin(alfatrig))*(1560*sin(Ttrignew) + 1860))/mass + g*sin(gam)) - lamGAM*((g*cos(gam))/v - (cos(bankmax*sin(banktrignew))*(g*mass + sin(alfamax*sin(alfatrig))*(1560*sin(Ttrignew) + 1860)))/(mass*v)) + (cos(gam)*(1560*sin(Ttrignew) + 1860)^5.2)/(v*(z + 50)^2.5) + lamX*v*cos(gam)*cos(psii) + lamY*v*cos(gam)*sin(psii) + (lamPSII*sin(bankmax*sin(banktrignew))*(g*mass + sin(alfamax*sin(alfatrig))*(1560*sin(Ttrignew) + 1860)))/(mass*v*cos(gam))','nd','Hamiltonian'};
in.figure(ind).plot(1).legend.location = 'NorthWest';
in.figure(ind).movie.make = false;
in.figure(ind).movie.name = 'testMovie';
in.figure(ind).movie.figPos = [1 31 1280 856*0.9];
