function sol = bvp4c_rdsl(ode, bc, solinit, options, varargin)
...
%   $Revision: 1.21.4.24 $  $Date: 2011/07/25 03:45:01 $
...
while ~done       
  ...
  for iter=1:maxNewtIter	
    if needGlobalJacobian
      ...
      wt = max(abs(dPHIdy),[],2);  
%       any(wt == 0)
%       all(isfinite(nonzeros(dPHIdy)))
      % dPHIdy
      % End of changes
      if any(wt == 0) && ~all(isfinite(nonzeros(dPHIdy)))
        singularJacobian = true;  
        disp('I am 1. wt = 0 and zeros issue with dPHIdy!!!!')
      elseif any(wt == 0)
        singularJacobian = true;  
        disp('I am in 2. Only wt = 0!!!!')  
       elseif ~all(isfinite(nonzeros(dPHIdy)))
        singularJacobian = true;  
        disp('I am in 3. Zeros issue with dPHIdy!!!!')  
      else
